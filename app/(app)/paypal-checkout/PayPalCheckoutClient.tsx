...
export default function PayPalCheckoutClient() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const callbackUrl = `${window.location.origin}/paypal/verify?purpose=${encodeURIComponent(
        purpose
      ...
      const res = await authFetch("/api/payments/paypal/init", {
        method: "POST",
      ...
      if (!res.ok || !json?.ok || !json.approval_url || !json.order_id) {
