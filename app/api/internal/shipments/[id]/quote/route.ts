...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [shipRows]: any = await conn.query(`SELECT * FROM linescout_shipments WHERE id = ? LIMIT 1`, [
      shipmentId,
    ]);
    ...
    let shippingRateUsd = num(shipment.shipping_rate_value, 0);
    let shippingRateUnit = String(shipment.shipping_rate_unit || "per_kg");

    if (!shippingRateUsd && shipment.shipping_rate_id) {
      const [rateRows]: any = await conn.query(
        `SELECT rate_value, rate_unit FROM linescout_shipping_rates WHERE id = ? LIMIT 1`,
        [shipment.shipping_rate_id]
      );
      if (rateRows?.length) {
        shippingRateUsd = num(rateRows[0].rate_value, 0);
    ...
    if (!shippingRateUsd || !units) {
      return NextResponse.json(
        { ok: false, error: "Shipping rate and units are required before creating the invoice." },
        { status: 400 }
      );
    }
    ...
    let email = String(shipment.contact_email || "").trim();
    if (!email && shipment.user_id) {
      const [userRows]: any = await conn.query(`SELECT email FROM users WHERE id = ? LIMIT 1`, [
        shipment.user_id,
      ]);
      email = String(userRows?.[0]?.email || "").trim();
    }

    const customerName =
      String(shipment.contact_name || "").trim() ||
      (email ? email.split("@")[0] : "Customer");

    const userPhone = await getUserPhone(conn, email || "");
