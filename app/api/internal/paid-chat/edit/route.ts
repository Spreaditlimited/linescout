...
/**
 * POST /api/internal/paid-chat/edit
 * body: { conversation_id, message_id, message_text }
 */
  ...
  try {
    ...
    if (auth.role !== "admin") {
      ...
      if (!isAssignedToAgent && !isClaimedByAgent) {
        return NextResponse.json(
          { ok: false, error: "You can read this project, but cannot edit messages." },
          { status: 403 }
        );
      }
    }
    ...
    if (m.deleted_at) {
      return NextResponse.json({ ok: false, error: "Message is deleted" }, { status: 400 });
    }
    const createdAt = m.created_at ? new Date(m.created_at).getTime() : 0;
    if (!createdAt || Date.now() - createdAt > 24 * 60 * 60 * 1000) {
      return NextResponse.json(
        { ok: false, error: "Editing window expired (24 hours)." },
        { status: 403 }
      );
    }

    const [attRows]: any = await conn.query(
      `SELECT id FROM linescout_message_attachments WHERE message_id = ? LIMIT 1`,
      [messageId]
    );
    if (attRows?.length) {
      return NextResponse.json(
        { ok: false, error: "Cannot edit messages with attachments" },
        { status: 400 }
      );
    }

    await conn.query(
      `UPDATE linescout_messages SET message_text = ?, edited_at = NOW() WHERE id = ?`,
    ...
    console.error("POST /api/internal/paid-chat/edit error:", e?.message || e);
    return NextResponse.json({ ok: false, error: "Failed to edit message" }, { status: 500 });
  } finally {
    conn.release();
  }
}
