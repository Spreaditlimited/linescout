...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => null)) as
    | { conversation_id?: number; last_seen_message_id?: number }
    | null;
  ...
  const conn = await db.getConnection();
  try {
    // Permission: admin can mark any. Agent can mark assigned-to-me OR unassigned.
    ...
    if (auth.role !== "admin") {
      if (assignedAgentId != null && assignedAgentId !== auth.userId) {
        return NextResponse.json({ ok: false, error: "You are not assigned to this conversation." }, { status: 403 });
      }
    }

    // Upsert read cursor (never move backwards)
    await conn.query(
      `
      INSERT INTO linescout_conversation_reads (conversation_id, internal_user_id, last_seen_message_id)
      VALUES (?, ?, ?)
      ON DUPLICATE KEY UPDATE
        last_seen_message_id = GREATEST(last_seen_message_id, VALUES(last_seen_message_id)),
        updated_at = CURRENT_TIMESTAMP
      `,
      [conversationId, auth.userId, Math.max(0, lastSeenId)]
    );

    return NextResponse.json({ ok: true, conversation_id: conversationId, last_seen_message_id: Math.max(0, lastSeenId) });
  } finally {
    conn.release();
  }
}