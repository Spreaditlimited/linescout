...
export async function POST(req: Request) {
  ...
  const emailEnabled = body?.email_notifications_enabled;
  const value = emailEnabled === true || emailEnabled === false ? emailEnabled : null;

  if (value === null) {
    return NextResponse.json(
      { ok: false, error: "email_notifications_enabled is required" },
      { status: 400 }
    );
  }
  ...
  try {
    await conn.query(
      ...
      LIMIT 1
      `,
      [value ? 1 : 0, auth.userId]
    );

    return NextResponse.json({ ok: true, email_notifications_enabled: value });
  } finally {
    conn.release();
  }
}
