...
export async function POST(req: Request) {
  try {
    ...
    await queryOne(
      `
      INSERT INTO linescout_device_tokens
        (user_id, platform, token, device_id, app_version, locale, is_active, last_seen_at)
      VALUES
        (?, ?, ?, ?, ?, ?, 1, NOW())
      ON DUPLICATE KEY UPDATE
        ...
        updated_at = NOW()
      `,
      [user.id, platform, token, device_id, app_version, locale]
    );
